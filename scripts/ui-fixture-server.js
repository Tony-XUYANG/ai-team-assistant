...
const database = {
  async listProjects() {
    const attention = entries.filter(entry => entry.is_current
      && ((entry.kind === "blocker" && entry.status === "open")
        || (entry.kind === "action" && ["todo", "doing"].includes(entry.status))
        || entry.verification === "unverified"))
      .slice(0, 3).map(({ id, project_id, kind, content, verification, status, owner_ref, occurred_at, created_at }) =>
        ({ id, project_id, kind, content, content_truncated: false, verification, status, owner_ref, occurred_at, created_at }));
    return { projects: [{ ...project, open_actions: entries.filter(e => e.is_current && e.kind === "action" && ["todo", "doing"].includes(e.status)).length, open_blockers: 0, unverified_entries: entries.filter(e => e.is_current && e.verification === "unverified").length, attention }], limit: 50, offset: 0, has_more: false, next_offset: null };
  },
